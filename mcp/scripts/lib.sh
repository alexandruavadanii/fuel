...
function create_networks {
  ...
  for i in $(seq 0 2 4); do
    sudo ip link add "veth_mcp$i" type veth peer name "veth_mcp$((i+1))" || true
    sudo ip link set "veth_mcp$i" up
    sudo ip link set "veth_mcp$((i+1))" up
    sudo brctl addif "${all_vnode_networks[$((i/2))]}" "veth_mcp$i" || true
  done
}
...
function prepare_containers {
  ...
  sudo rm -rf "${image_dir}/nodes/"*
  sudo rm -rf "${image_dir}/pki/"*
}
