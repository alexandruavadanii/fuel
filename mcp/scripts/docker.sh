#!/bin/bash -x
##############################################################################
# Copyright (c) 2018 EMC, Mirantis Inc., Enea AB and others.
#
...
function veth_prep {
  local FIXME
  sudo ip link add alav1 type veth peer name alav2
  sudo ifconfig alav1 up
  sudo ifconfig alav2 up
  sudo brctl addif mcpcontrol alav1
}
